...
#import sys
#sys.path.append('/home/oscar/bom_nnyse/nn_trade')
...
class NN_Network():
    ...
    def define(self):
        ...
        for net in self.ndef.network1A:
            xxx = self._layer(net,xxx, self.data_t.A_nvars_i, self.data_t.A_lookback)
    ...
    def run(self, save=False, ename=None):
        ...
        home = str(Path.home())
        if ename != None:
            dir = os.path.join(self.ndef.basedir+'/'+self.ndef.field+'/'+self.ndef.exp+ '/' + self.ndef.set+'/',ename)
            if not os.path.exists(dir): os.makedirs(dir)
        else:
            dir = os.path.join(self.ndef.basedir+'/'+self.ndef.field+'/'+self.ndef.exp+ '/' + self.ndef.set)

        mc = ModelCheckpoint(dir+ '/'+'best_model.h5',
                         monitor='val_loss', mode='min', verbose=0, save_best_only=True)

#       dir=home+'/bom_nnyse/nn_exps/' + self.name
...
#                                  callbacks=[es,mc,tensorboard_cb])
    ...
    def _layer(self, net, xxx, nvars, lookback):
        if net[0] == 'Dense':
            if len(net) < 3:
        ...
        if net[0] == 'Conv1D': xxx = layers.Conv1D(net[1], (net[2]), input_shape=(lookback,nvars),activation=net[3])(xxx)
        #if net[0] == 'Conv1D': xxx = layers.Conv1D(net[1], (net[2]), activation=net[3])(xxx)
        if net[0] == 'MaxPooling1D': xxx = layers.MaxPooling1D(net[1])(xxx)
        if net[0] == 'GlobalMaxPooling1D': xxx = layers.GlobalMaxPooling1D()(xxx)
...
def nn_main(ensemble,ndef, update_data=False, run_nn=True):
    ...
    if ensemble==0 or update_data == True:
        xx = NNDCube(ndef, ndef.dates_train, ndef.lookbackA, 'bom_nn', 't', 'A', ndef.varsiA, ndef.varso)
        xx = NNDCube(ndef, ndef.dates_val, ndef.lookbackA, 'bom_nn', 'v', 'A', ndef.varsiA, ndef.varso)
        xx = NNDCube(ndef, ndef.dates_ind, ndef.lookbackA, 'bom_nn', 'i', 'A', ndef.varsiA, ndef.varso)
        if ndef.n_nets == 2: 
            xx = NNDCube(ndef, ndef.dates_train, ndef.lookbackB, 'bom_nn', 't', 'B', ndef.varsiB, None)
            xx = NNDCube(ndef, ndef.dates_val, ndef.lookbackB, 'bom_nn', 'v', 'B', ndef.varsiB, None)
            xx = NNDCube(ndef, ndef.dates_ind, ndef.lookbackB, 'bom_nn', 'i', 'B', ndef.varsiB, None)
        if ndef.n_nets == 3: 
            xx = NNDCube(ndef, ndef.dates_train, ndef.lookbackC, 'bom_nn', 't', 'C', ndef.varsiB, None)
            xx = NNDCube(ndef, ndef.dates_val, ndef.lookbackC, 'bom_nn', 'v', 'C', ndef.varsiB, None)
            xx = NNDCube(ndef, ndef.dates_ind, ndef.lookbackC, 'bom_nn', 'i', 'C', ndef.varsiB, None)
